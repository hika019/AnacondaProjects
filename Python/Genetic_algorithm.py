...
def main():
    ...
    MUTINDPB = 0.1  # The probability of gene mutation.
    NGEN     = 50    # The number of generation loop.

    random.seed(64)
    # --- Step1 : Create initial generation.個体＆世代の作成
    pop = create_pop(n_ind, n_gene)
    set_fitness(evalOneMax, pop)
    best_ind = max(pop, key=attrgetter("fitness"))
    
    # --- Generation loop.世代のループ
    ...
    for g in range(NGEN):
        ...
        # --- Update next population.新たな個体集合
        pop = offspring[:]
        set_fitness(evalOneMax, pop)
